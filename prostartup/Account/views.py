...
def account(request):# Вход в админпанель 
    ...
    sub_data = Subscription.objects.get(user=request.user)
    payment_data = Personalaccount.objects.get(user = request.user)
    # group = Group.objects.all().filter(id = request.user.id)
    if request.method == 'POST':
    ...
    if has_group(request.user, "Seller"):
        ...
        offers = Offer.objects.filter(from_user__in=[request.user], deal = False)
    ...
    context1 = {'form_ref':form_ref, 
                'data_profile':data_profile, 
                'payment_data':payment_data,
                'groups': Group.objects.all().filter(user = request.user.id),
                'count' : Group.objects.all().filter(user = request.user.id).count(),
                'sub_data': sub_data,
                }
